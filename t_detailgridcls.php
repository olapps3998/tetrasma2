...
$t_detail_grid = NULL; // Initialize page object first

class ct_detail_grid extends ct_detail {
	...
	// Page object name
	var $PageObjName = 't_detail_grid';

	// Grid form hidden field names
	var $FormName = 'ft_detailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_detail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->detail_id->CurrentValue = strval($arKeys[0]); // detail_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_jurnal") {
			...
			if ($GLOBALS["t_jurnal"]->EventCancelled) $this->EventCancelled = TRUE;
		}
