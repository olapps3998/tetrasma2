...
$t_detail_view = NULL; // Initialize page object first

class ct_detail_view extends ct_detail {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["detail_id"] <> "") {
			$this->RecKey["detail_id"] = $_GET["detail_id"];
			$KeyUrl .= "&amp;detail_id=" . urlencode($this->RecKey["detail_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["detail_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["detail_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["detail_id"] <> "") {
				$this->detail_id->setFormValue($_POST["detail_id"]);
				$this->RecKey["detail_id"] = $this->detail_id->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "t_detaillist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "t_detaillist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($t_detail_view)) $t_detail_view = new ct_detail_view();

// Page init
$t_detail_view->Page_Init();

// Page main
$t_detail_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_detail_view->Page_Render();
?>
...
var CurrentForm = ft_detailview = new ew_Form("ft_detailview", "view");

// Form_CustomValidate event
ft_detailview.Form_CustomValidate = 
...
ft_detailview.ValidateRequired = true;
<?php } else { ?>
ft_detailview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_detailview.Lists["x_coa_id"] = {"LinkField":"x_coal4_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_coal4_no","x_coal4_nm","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_coal4"};
...
<?php if ($t_detail->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_detail_view->IsModal) { ?>
<?php if ($t_detail->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_detail_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_detail_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_detail_view->IsModal) { ?>
<?php if ($t_detail->Export == "") { ?>
...
<?php } ?>
<?php $t_detail_view->ShowPageHeader(); ?>
<?php
$t_detail_view->ShowMessage();
?>
<?php if (!$t_detail_view->IsModal) { ?>
<?php if ($t_detail->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_detail_view->Pager)) $t_detail_view->Pager = new cPrevNextPager($t_detail_view->StartRec, $t_detail_view->DisplayRecs, $t_detail_view->TotalRecs) ?>
<?php if ($t_detail_view->Pager->RecordCount > 0 && $t_detail_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_detail_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_detail_view->PageUrl() ?>start=<?php echo $t_detail_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_detail_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_detail_view->PageUrl() ?>start=<?php echo $t_detail_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_detail_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_detail_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_detail_view->PageUrl() ?>start=<?php echo $t_detail_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_detail_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_detail_view->PageUrl() ?>start=<?php echo $t_detail_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_detail_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<?php } ?>
<form name="ft_detailview" id="ft_detailview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_detail_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_detail_view->Token ?>">
<?php } ?>
...
<?php if ($t_detail->jurnal_id->Visible) { // jurnal_id ?>
	<tr id="r_jurnal_id">
		<td><span id="elh_t_detail_jurnal_id"><?php echo $t_detail->jurnal_id->FldCaption() ?></span></td>
...
<?php echo $t_detail->jurnal_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_detail->coa_id->Visible) { // coa_id ?>
	<tr id="r_coa_id">
		<td><span id="elh_t_detail_coa_id"><?php echo $t_detail->coa_id->FldCaption() ?></span></td>
...
<?php echo $t_detail->coa_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_detail->debet->Visible) { // debet ?>
	<tr id="r_debet">
		<td><span id="elh_t_detail_debet"><?php echo $t_detail->debet->FldCaption() ?></span></td>
...
<?php echo $t_detail->debet->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_detail->kredit->Visible) { // kredit ?>
	<tr id="r_kredit">
		<td><span id="elh_t_detail_kredit"><?php echo $t_detail->kredit->FldCaption() ?></span></td>
...
<?php echo $t_detail->kredit->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_detail->anggota_id->Visible) { // anggota_id ?>
	<tr id="r_anggota_id">
		<td><span id="elh_t_detail_anggota_id"><?php echo $t_detail->anggota_id->FldCaption() ?></span></td>
...
<?php echo $t_detail->anggota_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
