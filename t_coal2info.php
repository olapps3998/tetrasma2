...
//
// Table class for t_coal2
//
class ct_coal2 extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_coal2';
		$this->TableName = 't_coal2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_coal2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// coal2_id
		$this->coal2_id = new cField('t_coal2', 't_coal2', 'x_coal2_id', 'coal2_id', '`coal2_id`', '`coal2_id`', 3, -1, FALSE, '`coal2_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// coal1_id
		$this->coal1_id = new cField('t_coal2', 't_coal2', 'x_coal1_id', 'coal1_id', '`coal1_id`', '`coal1_id`', 3, -1, FALSE, '`EV__coal1_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// coal2_no
		$this->coal2_no = new cField('t_coal2', 't_coal2', 'x_coal2_no', 'coal2_no', '`coal2_no`', '`coal2_no`', 200, -1, FALSE, '`coal2_no`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->coal2_no->Sortable = TRUE; // Allow sort
		$this->fields['coal2_no'] = &$this->coal2_no;

		// coal2_nm
		$this->coal2_nm = new cField('t_coal2', 't_coal2', 'x_coal2_nm', 'coal2_nm', '`coal2_nm`', '`coal2_nm`', 200, -1, FALSE, '`coal2_nm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->coal2_nm->Sortable = TRUE; // Allow sort
		$this->fields['coal2_nm'] = &$this->coal2_nm;
	}
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "t_coal1") {
			if ($this->coal1_id->getSessionValue() <> "")
				$sMasterFilter .= "`coal1_id`=" . ew_QuotedValue($this->coal1_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "t_coal1") {
			if ($this->coal1_id->getSessionValue() <> "")
				$sDetailFilter .= "`coal1_id`=" . ew_QuotedValue($this->coal1_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_t_coal1() {
		return "`coal1_id`=@coal1_id@";
	}

	// Detail filter
	function SqlDetailFilter_t_coal1() {
		return "`coal1_id`=@coal1_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_coal2`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->coal2_id->setDbValue($conn->Insert_ID());
			$rs['coal2_id'] = $this->coal2_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('coal2_id', $rs))
				ew_AddFilter($where, ew_QuotedName('coal2_id', $this->DBID) . '=' . ew_QuotedValue($rs['coal2_id'], $this->coal2_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`coal2_id` = @coal2_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->coal2_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@coal2_id@", ew_AdjustSql($this->coal2_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_coal2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_coal2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_coal2add.php?" . $this->UrlParm($parm);
		else
			$url = "t_coal2add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_coal2delete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t_coal1" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
	...
	function KeyToJson() {
		$json = "";
		$json .= "coal2_id:" . ew_VarToJson($this->coal2_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->coal2_id->CurrentValue)) {
			$sUrl .= "coal2_id=" . urlencode($this->coal2_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["coal2_id"]))
				$arKeys[] = ew_StripSlashes($_POST["coal2_id"]);
			elseif (isset($_GET["coal2_id"]))
				$arKeys[] = ew_StripSlashes($_GET["coal2_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->coal2_id->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->coal2_id->EditValue = $this->coal2_id->CurrentValue;
		$this->coal2_id->ViewCustomAttributes = "";
		...
		$this->coal2_no->EditValue = $this->coal2_no->CurrentValue;
		...
		$this->coal2_nm->EditValue = $this->coal2_nm->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->coal2_no->Exportable) $Doc->ExportCaption($this->coal2_no);
					if ($this->coal2_nm->Exportable) $Doc->ExportCaption($this->coal2_nm);
				} else {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->coal2_no->Exportable) $Doc->ExportField($this->coal2_no);
						if ($this->coal2_nm->Exportable) $Doc->ExportField($this->coal2_nm);
					} else {
