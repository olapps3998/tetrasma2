...
$t_detail_delete = NULL; // Initialize page object first

class ct_detail_delete extends ct_detail {
	...
	// Page object name
	var $PageObjName = 't_detail_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_detaillist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_detail class, t_detailinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_detaillist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_detail_delete)) $t_detail_delete = new ct_detail_delete();

// Page init
$t_detail_delete->Page_Init();

// Page main
$t_detail_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_detail_delete->Page_Render();
?>
...
var CurrentForm = ft_detaildelete = new ew_Form("ft_detaildelete", "delete");

// Form_CustomValidate event
ft_detaildelete.Form_CustomValidate = 
...
ft_detaildelete.ValidateRequired = true;
<?php } else { ?>
ft_detaildelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_detaildelete.Lists["x_coa_id"] = {"LinkField":"x_coal4_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_coal4_no","x_coal4_nm","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_coal4"};
...
<?php $t_detail_delete->ShowPageHeader(); ?>
<?php
$t_detail_delete->ShowMessage();
?>
<form name="ft_detaildelete" id="ft_detaildelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_detail_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_detail_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_detail">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_detail_delete->RecKeys as $key) { ?>
...
<?php if ($t_detail->jurnal_id->Visible) { // jurnal_id ?>
		<th><span id="elh_t_detail_jurnal_id" class="t_detail_jurnal_id"><?php echo $t_detail->jurnal_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_detail->coa_id->Visible) { // coa_id ?>
		<th><span id="elh_t_detail_coa_id" class="t_detail_coa_id"><?php echo $t_detail->coa_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_detail->debet->Visible) { // debet ?>
		<th><span id="elh_t_detail_debet" class="t_detail_debet"><?php echo $t_detail->debet->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_detail->kredit->Visible) { // kredit ?>
		<th><span id="elh_t_detail_kredit" class="t_detail_kredit"><?php echo $t_detail->kredit->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_detail->anggota_id->Visible) { // anggota_id ?>
		<th><span id="elh_t_detail_anggota_id" class="t_detail_anggota_id"><?php echo $t_detail->anggota_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_detail_delete->RecCnt = 0;
$i = 0;
while (!$t_detail_delete->Recordset->EOF) {
	$t_detail_delete->RecCnt++;
	$t_detail_delete->RowCnt++;

	// Set row properties
	$t_detail->ResetAttrs();
	$t_detail->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_detail_delete->LoadRowValues($t_detail_delete->Recordset);

	// Render row
	$t_detail_delete->RenderRow();
?>
	<tr<?php echo $t_detail->RowAttributes() ?>>
<?php if ($t_detail->jurnal_id->Visible) { // jurnal_id ?>
		<td<?php echo $t_detail->jurnal_id->CellAttributes() ?>>
<span id="el<?php echo $t_detail_delete->RowCnt ?>_t_detail_jurnal_id" class="t_detail_jurnal_id">
...
<?php } ?>
<?php if ($t_detail->coa_id->Visible) { // coa_id ?>
		<td<?php echo $t_detail->coa_id->CellAttributes() ?>>
<span id="el<?php echo $t_detail_delete->RowCnt ?>_t_detail_coa_id" class="t_detail_coa_id">
...
<?php } ?>
<?php if ($t_detail->debet->Visible) { // debet ?>
		<td<?php echo $t_detail->debet->CellAttributes() ?>>
<span id="el<?php echo $t_detail_delete->RowCnt ?>_t_detail_debet" class="t_detail_debet">
...
<?php } ?>
<?php if ($t_detail->kredit->Visible) { // kredit ?>
		<td<?php echo $t_detail->kredit->CellAttributes() ?>>
<span id="el<?php echo $t_detail_delete->RowCnt ?>_t_detail_kredit" class="t_detail_kredit">
...
<?php } ?>
<?php if ($t_detail->anggota_id->Visible) { // anggota_id ?>
		<td<?php echo $t_detail->anggota_id->CellAttributes() ?>>
<span id="el<?php echo $t_detail_delete->RowCnt ?>_t_detail_anggota_id" class="t_detail_anggota_id">
...
<?php
	$t_detail_delete->Recordset->MoveNext();
}
$t_detail_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_detail_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_detaildelete.Init();
</script>
<?php
$t_detail_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_detail_delete->Page_Terminate();
?>
