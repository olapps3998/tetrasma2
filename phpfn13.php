...
function CurrentProjectID() {
	...
	return "{BD598998-6524-4166-9FBE-52F174C8EABD}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('user_pw'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('user_nm')); // Load user name
						$this->setSessionUserID($rs->fields('user_id')); // Load User ID
						if (is_null($rs->fields('user_lv'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('user_lv'))); // Load User Level
						}
						$this->SetUpUserLevel();
