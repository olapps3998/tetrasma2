...
//
// Table class for t_detail
//
class ct_detail extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_detail';
		$this->TableName = 't_detail';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_detail`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// detail_id
		$this->detail_id = new cField('t_detail', 't_detail', 'x_detail_id', 'detail_id', '`detail_id`', '`detail_id`', 3, -1, FALSE, '`detail_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jurnal_id
		$this->jurnal_id = new cField('t_detail', 't_detail', 'x_jurnal_id', 'jurnal_id', '`jurnal_id`', '`jurnal_id`', 3, -1, FALSE, '`jurnal_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// coa_id
		$this->coa_id = new cField('t_detail', 't_detail', 'x_coa_id', 'coa_id', '`coa_id`', '`coa_id`', 3, -1, FALSE, '`EV__coa_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		$this->coa_id->Sortable = TRUE; // Allow sort
		$this->coa_id->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->coa_id->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->coa_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['coa_id'] = &$this->coa_id;

		// debet
		$this->debet = new cField('t_detail', 't_detail', 'x_debet', 'debet', '`debet`', '`debet`', 4, -1, FALSE, '`debet`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// kredit
		$this->kredit = new cField('t_detail', 't_detail', 'x_kredit', 'kredit', '`kredit`', '`kredit`', 4, -1, FALSE, '`kredit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// anggota_id
		$this->anggota_id = new cField('t_detail', 't_detail', 'x_anggota_id', 'anggota_id', '`anggota_id`', '`anggota_id`', 3, -1, FALSE, '`anggota_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "t_jurnal") {
	...
	// Master filter
	function SqlMasterFilter_t_jurnal() {
		return "`jurnal_id`=@jurnal_id@";
	}

	// Detail filter
	function SqlDetailFilter_t_jurnal() {
		return "`jurnal_id`=@jurnal_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_detail`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`coal4_no`,'" . ew_ValueSeparator(1, $this->coa_id) . "',`coal4_nm`) FROM `t_coal4` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`coal4_id` = `t_detail`.`coa_id` LIMIT 1) AS `EV__coa_id` FROM `t_detail`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if ($this->coa_id->AdvancedSearch->SearchValue <> "" ||
			$this->coa_id->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->coa_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->coa_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->detail_id->setDbValue($conn->Insert_ID());
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_detailview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_detailview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_detailadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_detailadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_detailedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_detailadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_detaildelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t_jurnal" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// coa_id
		$this->coa_id->EditAttrs["class"] = "form-control";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->coa_id->Exportable) $Doc->ExportCaption($this->coa_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->coa_id->Exportable) $Doc->ExportField($this->coa_id);
