...
$t_jurnal_view = NULL; // Initialize page object first

class ct_jurnal_view extends ct_jurnal {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "t_jurnallist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "t_jurnallist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_t_detail"
		$item = &$option->Add("detail_t_detail");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("t_detail", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["t_detail_grid"] && $GLOBALS["t_detail_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 't_detail')) {
		...
		if ($GLOBALS["t_detail_grid"] && $GLOBALS["t_detail_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 't_detail')) {
		...
		if ($GLOBALS["t_detail_grid"] && $GLOBALS["t_detail_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 't_detail')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 't_detail');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (t_detail)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("t_detail", explode(",", $this->getCurrentDetailTable()))) {
			global $t_detail;
			if (!isset($t_detail)) $t_detail = new ct_detail;
			$rsdetail = $t_detail->LoadRs($t_detail->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$t_detail->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_detail", $DetailTblVar)) {
				...
				if ($GLOBALS["t_detail_grid"]->DetailView) {
					$GLOBALS["t_detail_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($t_jurnal_view)) $t_jurnal_view = new ct_jurnal_view();

// Page init
$t_jurnal_view->Page_Init();

// Page main
$t_jurnal_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_jurnal_view->Page_Render();
?>
...
var CurrentForm = ft_jurnalview = new ew_Form("ft_jurnalview", "view");

// Form_CustomValidate event
ft_jurnalview.Form_CustomValidate = 
...
ft_jurnalview.ValidateRequired = true;
<?php } else { ?>
ft_jurnalview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($t_jurnal->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_jurnal_view->IsModal) { ?>
<?php if ($t_jurnal->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_jurnal_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_jurnal_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_jurnal_view->IsModal) { ?>
<?php if ($t_jurnal->Export == "") { ?>
...
<?php } ?>
<?php $t_jurnal_view->ShowPageHeader(); ?>
<?php
$t_jurnal_view->ShowMessage();
?>
<?php if (!$t_jurnal_view->IsModal) { ?>
<?php if ($t_jurnal->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_jurnal_view->Pager)) $t_jurnal_view->Pager = new cPrevNextPager($t_jurnal_view->StartRec, $t_jurnal_view->DisplayRecs, $t_jurnal_view->TotalRecs) ?>
<?php if ($t_jurnal_view->Pager->RecordCount > 0 && $t_jurnal_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_jurnal_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_jurnal_view->PageUrl() ?>start=<?php echo $t_jurnal_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_jurnal_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_jurnal_view->PageUrl() ?>start=<?php echo $t_jurnal_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_jurnal_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_jurnal_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_jurnal_view->PageUrl() ?>start=<?php echo $t_jurnal_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_jurnal_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_jurnal_view->PageUrl() ?>start=<?php echo $t_jurnal_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_jurnal_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<?php } ?>
<form name="ft_jurnalview" id="ft_jurnalview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_jurnal_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_jurnal_view->Token ?>">
<?php } ?>
...
<?php if ($t_jurnal->no_bukti->Visible) { // no_bukti ?>
	<tr id="r_no_bukti">
		<td><span id="elh_t_jurnal_no_bukti"><?php echo $t_jurnal->no_bukti->FldCaption() ?></span></td>
...
<?php echo $t_jurnal->no_bukti->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("t_detail", explode(",", $t_jurnal->getCurrentDetailTable())) && $t_detail->DetailView) {
?>
